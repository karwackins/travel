            ...
            <ul class="list-inline">
                @foreach($object->users as $user)
                    <li><a href="{{ route('person') }}">
                            <img title="{{$user->FullName}}" class="media-object img-responsive" width="50" height="50" src="{{$user->photos->first()->path}}" alt=""> </a></li>
                @endforeach
            </ul>

        </div>
        <div class="tab-pane fade" id="adress">
            <p>{{$object->city->name}} - {{$object->address->street}} {{$object->address->number}}</p>
        </div>
    </div>
            ...
            <div class="media">
                <div class="media-left media-top">
                    ...
                    <a title="{{$comment->user->FullName}}" href="{{ route('person') }}">
                        <img class="media-object" width="50" height="50" src="{{$comment->user->photos->first()->path}}" alt="...">
                    </a>
                </div>
    ...
    <section>
        ...
        @foreach($object->articles as $article)
            <div class="articles-list">
                ...
                <p>{{$article->content}}</p> <a href="{{ route('article') }}">More</a>
            </div>
